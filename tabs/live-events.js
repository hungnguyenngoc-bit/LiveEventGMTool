...
const DEFAULT_TAB1_ZOOM_PERCENT = 50;
const TRACK_COLORS = [
  "#5B8FF9",
  "#5AD8A6",
  "#F6BD16",
...
async function bootstrap() {
  ...
  const zoomPercentRaw =
    env.TAB1_DEFAULT_ZOOM_PERCENT ?? env.DEFAULT_TAB1_ZOOM_PERCENT ?? env.DEFAULT_ZOOM_PERCENT;
  if (zoomPercentRaw !== undefined) {
    const parsed = Number(zoomPercentRaw);
    if (Number.isFinite(parsed)) {
      const percent = clampZoomPercent(parsed);
      state.zoom = (BASE_ZOOM * percent) / 100;
    }
  } else {
    state.zoom = (BASE_ZOOM * DEFAULT_TAB1_ZOOM_PERCENT) / 100;
  }
